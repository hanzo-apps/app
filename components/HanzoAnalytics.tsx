...
//
// With no NEXT_PUBLIC_ANALYTICS_WEBSITE_ID set the tag does not load (hanzo.app
// has no analytics property yet — create one at analytics.hanzo.ai and set the
// env var). GA/Pixel likewise load only when their ids are provided — no
// fabricated ids. Funnel/custom events go through trackEvent() below.
...
export function HanzoAnalytics() {
  // No website id -> nothing to track yet. Render nothing (no bare script).
 ...
 * module. The native tracker forwards it to Hanzo analytics AND (when the id is
 * set) to GA4 (`gtag('event', name, props)`) and the Meta Pixel
 * (`fbq('trackCustom', name, props)`). Safe to call before the script loads.
 */
