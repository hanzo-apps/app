...
>
  {#if _doubleRow}
    <div class="hanzoHeader-row">
      {#if allowFullsize}
        ...
        />
        <div class="hanzoHeader-divider no-print" />
      {/if}
      {#if $$slots.beforeTitle && !hideBefore}
        <div class="hanzoHeader-buttonsGroup before mr-2 no-print" class:freezeBefore>
      ...
      >
        {#if $$slots.description && !hideDescription}
          <div class="hanzoHeader-titleGroup"><slot /></div>
          <div class="hanzoHeader-titleGroup"><slot name="description" /></div>
        {:else}
          <slot />
        {/if}
      </div>
      ...
      {#if $$slots.actions && !hideActions && (adaptive === 'freezeActions' || adaptive === 'doubleRow' || adaptive === 'autoExtra')}
        <div class="hanzoHeader-buttonsGroup actions no-print">
          <slot name="actions" {doubleRow} />
        </div>
      {/if}
      {#if closeButton}
        {#if type !== 'type-popup'}<div class="hanzoHeader-divider no-print" />{/if}
        {#if closeOnEscape}
          <div class="hanzoHotKey-item no-print">Esc</div>
        {/if}
        <ButtonIcon icon={IconClose} kind={'tertiary'} size={'small'} noPrint on:click={() => dispatch('close')} />
      {/if}
    </div>
    <!-- <div class="hanzoHeader-row__divider" /> -->
    <div class="hanzoHeader-row no-print" class:between={$$slots.search} class:reverse={!$$slots.search}>
      {#if $$slots.presence && !hidePresence}
        <div class="hanzoHeader-buttonsGroup presence no-print">
          <slot name="presence" {doubleRow} />
        </div>
      {/if}
      {#if $$slots.search}
        <div class="hanzoHeader-buttonsGroup search">
          <slot name="search" {doubleRow} />
        </div>
      {/if}
      {#if $$slots.extra && !hideExtra && (adaptive === 'doubleRow' || adaptive === 'autoExtra')}
        <div
          class="hanzoHeader-buttonsGroup extra"
      ...
      {/if}
      {#if $$slots.actions && !hideActions && !(adaptive === 'freezeActions' || adaptive === 'doubleRow' || adaptive === 'autoExtra')}
        <div class="hanzoHeader-buttonsGroup actions">
          <slot name="actions" {doubleRow} />
        </div>
      {/if}
    </div>
  {:else}
    ...
    {/if}
    {#if $$slots.beforeTitle && !hideBefore}
      <div class="hanzoHeader-buttonsGroup before mr-2 no-print">
        <slot name="beforeTitle" {doubleRow} />
      </div>
    {/if}
    <!-- svelte-ignore a11y-click-events-have-key-events -->
    ...
    {#if $$slots.search && !hideSearch}
      <div class="hanzoHeader-buttonsGroup search no-print">
        <slot name="search" {doubleRow} />
      </div>
      {#if $$slots.actions && !hideActions}<div class="hanzoHeader-divider no-print" />{/if}
    {/if}
