<!--
// Copyright © 2021, 2023 Hanzo Platform Contributors.
//
...
<button
  class="hanzoNavItem-container line-height-auto {type} {type === 'type-anchor-link'
    ? 'font-regular-12'
    : 'font-regular-14'}"
  class:selected
  class:bold
...
>
  ...
  {#if visibleIcon || (type === 'type-tag' && color)}
    <div class="hanzoNavItem-icon" class:withBackground class:w-auto={iconSize === 'x-small'}>
      {#if type !== 'type-tag' && visibleIcon}
        <Icon icon={visibleIcon} size={iconSize} {iconProps} />
      {:else if type === 'type-tag'}
        <div style:background-color={color} class="hanzoNavItem-icon__tag" />
      {/if}
    </div>
  {/if}
  <span
    ...
    class="{description ? 'hanzoNavItem-wideLabel' : 'hanzoNavItem-label'} overflow-label"
    class:flex-grow={!(type === 'type-anchor-link')}
    style:color={type === 'type-tag' && selected ? color : null}
  >
    {#if description}
      <span class="hanzoNavItem-label font-medium-12 line-height-auto mr-0-5">
  ...
  {#if showMenu || $$slots.actions}
    <div class="hanzoNavItem-actions">
      <slot name="actions" />
    </div>
  {/if}
  {#if count !== null}
    <span class="hanzoNavItem-count font-bold-12">
      {count}
    </span>
  {/if}
  <slot name="notify" />
...
{#if (isFold && (isOpen || (!isOpen && visible)) && !empty) || forciblyСollapsed}
  <div class="hanzoNavItem-dropbox">
  ...
  .hanzoNavItem-container {
    ...
    &.type-link {
      ...
      &.selected {
        ...
        .hanzoNavItem-label:not(.description) {
          color: var(--global-accent-TextColor);
        }
      }
    }
    ...
    &.type-anchor-link {
      ...
      &.selected .hanzoNavItem-icon,
      &.selected .hanzoNavItem-label {
        color: var(--global-primary-TextColor);
      }
    }
    &.indent {
    ...
    &:hover .hanzoNavItem-chevron:enabled {
      color: var(--global-secondary-TextColor);
      background-color: var(--button-tertiary-hover-BackgroundColor);
    }

    &:not(.noActions):hover .hanzoNavItem-actions,
    &:not(.noActions).showMenu .hanzoNavItem-actions {
      display: flex;
    }
    &.disabled {
      cursor: not-allowed;
