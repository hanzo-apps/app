...
// the themes this app actually uses. Run after changing lib/gui.ts:
...
// lib/gui.ts is the ONE gui config — it re-exports the same object @hanzo/ui
// registers, so the sheet generated here and the styles the components ask for
// come from a single token table. It exports `config as default`, hence the
// default import.
execSync(
  'npx esbuild lib/gui.ts --bundle --format=esm --platform=node --external:react --external:react-dom --external:react-native --alias:react-native=react-native-web --outfile=.guicfg.gen.mjs --log-level=error',
  { stdio: 'inherit' }
)
