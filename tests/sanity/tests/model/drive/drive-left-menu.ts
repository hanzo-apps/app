...
export class DriveLeftMenu {
  ...
  readonly leftMenu = (): Locator => this.page.locator('.antiPanel-wrap__content.hanzoNavPanel-container')
  ...
  readonly buttonDriveInTree = (drive: Drive): Locator =>
    this.treeDrives().locator('.hanzoNavGroup-container').getByRole('button', { name: drive.name })

  readonly driveContainer = (drive: Drive): Locator =>
    this.treeDrives()
      .locator('.hanzoNavGroup-container')
      .filter({
