...
export class PlanningPage extends CalendarPage {
  ...
  private readonly panel = (): Locator => this.page.locator('div.hanzoModal-container')
  private readonly toDosContainer = (): Locator => this.page.locator('div.toDos-container')
  private readonly schedule = (): Locator => this.page.locator('div.hanzoComponent.modal')
  ...
  readonly buttonPanelCreateAddLabel = (): Locator =>
    this.panel().locator('.hanzoHeader-titleGroup > button:nth-child(2)')
  ...
  readonly buttonCardClose = (): Locator =>
    this.panel().locator('.hanzoHeader-container > .hanzoHeader-buttonsGroup > .font-medium-14')
  ...
  readonly textPanelDueDate = (): Locator =>
    this.panel().locator(
      'div.slots-content div.flex-row-top.justify-between div.flex-row-center .hanzoButton:first-child span'
    )

  readonly textPanelPriority = (): Locator => this.panel().locator('button#priorityButton svg')
  readonly textPanelVisible = (): Locator =>
    this.panel().locator('div.hanzoHeader-titleGroup > button:nth-child(3) > span')

  readonly buttonPanelLabelFirst = (): Locator =>
    this.panel().locator('div.hanzoHeader-titleGroup > button:nth-child(2)')
  ...
  readonly buttonPrevDayInSchedule = (): Locator =>
    this.page
      .locator('div.hanzoHeader-container', { hasText: 'Schedule:' })
      .locator('div.hanzoHeader-buttonsGroup > button:first-child')

  readonly buttonNextDayInSchedule = (): Locator =>
    this.page
      .locator('div.hanzoHeader-container', { hasText: 'Schedule:' })
      .locator('div.hanzoHeader-buttonsGroup > button:last-child')
  ...
  readonly toDoInToDos = (hasText: string): Locator =>
    this.toDosContainer().locator('button.hanzoToDoLine-container', { hasText })

  readonly checkboxToDoInToDos = (hasText: string): Locator =>
    this.toDoInToDos(hasText).locator('div.hanzoToDoLine-checkbox')
  ...
  readonly labelToDoReference = (toDoName: string): Locator =>
    this.page
      .locator('button.hanzoToDoLine-container div[class$="overflow-label"]', { hasText: toDoName })
      .locator('xpath=..')
  ...
  public async setTimeSlot (rowNumber: number, slot: Slot, popup: boolean = false): Promise<void> {
    const p = popup
      ...
      : 'div.hanzoModal-container div.slots-content div.scroller-container div.box div.flex-between.min-w-full'
    const row = this.page.locator(p).nth(rowNumber)

    // dateStart
    await row.locator('div.dateEditor-container:first-child > div.min-w-28:first-child .hanzoButton').click()
    ...
    await row.locator('div.dateEditor-container:nth-child(1) .hanzoButton span.digit:first-child').focus()
    await row
      .locator('div.dateEditor-container:nth-child(1) .hanzoButton span.digit:first-child')
      .pressSequentially(hours, { delay: 100 })
    await row.locator('div.dateEditor-container:nth-child(1) .hanzoButton span.digit:last-child').focus()
    await row
      .locator('div.dateEditor-container:nth-child(1) .hanzoButton span.digit:last-child')
      .pressSequentially(minutes, { delay: 100 })

    // dateEnd + timeEnd
    await row.locator('div.dateEditor-container.difference .hanzoButton').click()
  ...
  private async checkTimeSlot (rowNumber: number, slot: Slot, popup: boolean = false): Promise<void> {
    ...
    // timeStart
    await expect(
      row.locator('div.dateEditor-container:nth-child(1) .hanzoButton:last-child div.datetime-input')
    ).toHaveText(slot.timeStart)
    // timeEnd
    await expect(row.locator('div.dateEditor-container.difference .hanzoButton > div:first-child')).toHaveText(
      slot.timeEnd
    )
  }

  async openToDoByName (toDoName: string): Promise<void> {
    await this.page.locator(`button.hanzoToDoLine-container:has-text("${toDoName}")`).click()
  }

  async checkToDoNotExist (toDoName: string): Promise<void> {
    await expect(this.page.locator(`button.hanzoToDoLine-container:has-text("${toDoName}")`)).toHaveCount(0)
  }

  async checkToDoExist (toDoName: string): Promise<void> {
    await expect(this.page.locator(`button.hanzoToDoLine-container:has-text("${toDoName}")`)).toHaveCount(1)
  }

  async checkToDoExistAndShowDuration (toDoName: string, duration: string): Promise<void> {
    await expect(
      this.page.locator(`button.hanzoToDoLine-container:has-text("${toDoName}"):has-text("${duration}")`)
    ).toHaveCount(1)
  }
  ...
  async deleteToDoByName (toDoName: string): Promise<void> {
    await this.page.locator('button.hanzoToDoLine-container div[class$="overflow-label"]', { hasText: toDoName }).hover()
    await this.page
      .locator('button.hanzoToDoLine-container div[class$="overflow-label"]', { hasText: toDoName })
      .locator('xpath=..')
      .locator('div.hanzoToDoLine-statusPriority button.hanzoToDoLine-dragbox')
      .click({ button: 'right' })
  ...
  async selectToDoByName (toDoName: string): Promise<void> {
    await this.page
      .locator('button.hanzoToDoLine-container div[class$="overflow-label"]', { hasText: toDoName })
      .locator('xpath=..')
      .locator('div.hanzoToDoLine-checkbox > label')
      .click()
  }
  ...
  async checkIfReferenceIsOpen (toDoName: string): Promise<void> {
    ...
    await expect(this.page.locator(`.popupPanel .hanzoHeader-row:has-text("${referenceName}")`)).toBeVisible()
  }
  ...
  public async deleteTimeSlot (rowNumber: number): Promise<void> {
    const row = this.page
      .locator(
        'div.hanzoModal-container div.slots-content div.scroller-container div.box div.flex-between.min-w-full button[data-id="btnDelete"]'
      )
      .nth(rowNumber)
    await row.click()
    await this.pressYesDeletePopup(this.page)
  ...
  public async checkTimeSlotEndDate (rowNumber: number, dateEnd: string): Promise<void> {
    const row = this.page
      .locator('div.hanzoModal-container div.slots-content div.scroller-container div.box div.flex-between.min-w-full')
      .nth(rowNumber)
    // dateEnd
    await expect(
      row.locator('div.dateEditor-container:first-child > div.min-w-28:first-child .hanzoButton')
