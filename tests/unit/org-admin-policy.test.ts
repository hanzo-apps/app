...
describe('isStaffAdmin — who may edit live', () => {
  it('the real owner identity is staff (the regression that was false for everyone)', () => {
    expect(isStaffAdmin({ org: 'hanzo', email: 'z@hanzo.ai' })).toBe(true);
  });

  it('ANY person in the admin org is staff — membership alone, no claim', () => {
    expect(isStaffAdmin({ org: 'admin' })).toBe(true);
  });

  it('an IAM org-admin of any brand org is staff', () => {
    for (const org of BRAND_ORGS) {
      expect(isStaffAdmin({ org: org, isAdmin: true })).toBe(true);
    }
  });

  it('a verified staff email is staff regardless of home org', () => {
    expect(isStaffAdmin({ org: 'acme', email: 'Z@Hanzo.AI' })).toBe(true);
  });

  it('a customer is NOT staff', () => {
    expect(isStaffAdmin({ org: 'acme', email: 'dave@acme.com' })).toBe(false);
    expect(isStaffAdmin({ org: 'acme', isAdmin: true, email: 'dave@acme.com' })).toBe(false);
    expect(isStaffAdmin({})).toBe(false);
  });

  it('a look-alike domain is NOT staff', () => {
    expect(isStaffAdmin({ org: 'acme', email: 'a@nothanzo.ai' })).toBe(false);
    expect(isStaffAdmin({ org: 'acme', email: 'a@hanzo.ai.evil.com' })).toBe(false);
  });
});

describe('isSuperAdmin — who may cross tenants', () => {
  it('ANY person in the admin org, with no further claim', () => {
    expect(isSuperAdmin({ org: 'admin' })).toBe(true);
  });

  it('ONLY the admin org — no brand org, org-admin flag, or staff email grants it', () => {
    expect(isSuperAdmin({ org: 'hanzo', isAdmin: true, email: 'z@hanzo.ai' })).toBe(false);
    for (const org of BRAND_ORGS) {
      expect(isSuperAdmin({ org: org, isAdmin: true, email: `x@${'hanzo.ai'}` })).toBe(false);
    }
    expect(isSuperAdmin({})).toBe(false);
  });

  it('is strictly narrower than staff — widening edit never widens scope', () => {
    const staffNotSudo = { org: 'hanzo', email: 'z@hanzo.ai' };
