 ...
 *         schema (DDL passed in the body, as held by the Schema editor).
 *
 * This is what turns "enable backend" into a real, persistent, IAM-native data
 * layer for a generated app.
 */

import { NextRequest, NextResponse } from 'next/server';
import { baseClientForRequest } from '@/lib/base/server';
import { isBaseConfigured } from '@/lib/base/config';
...
export async function GET(): Promise<NextResponse> {
  ...
  try {
    ...
    return NextResponse.json({ configured: true, authenticated: true, collections: [], error: 'Failed to read Base' }, { status: 502 });
  }
}
