 ...
 *
 * Keeping the token server-side means the browser never holds a Base
 * credential, and there is no cross-origin call from the preview/deployed app.
 */

import { NextRequest, NextResponse } from 'next/server';
import { baseUrl, isBaseConfigured } from '@/lib/base/config';
...
async function handle(request: NextRequest, { params }: RouteParams): Promise<NextResponse> {
  ...
  const upstream = `${baseUrl()}/${path.map(encodeURIComponent).join('/')}${search}`;
