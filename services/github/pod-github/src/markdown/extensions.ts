...
export interface SubLinkOptions {
  ...
  hasHanzoText: (text: string) => boolean
  hasHanzoLink: (href: string) => boolean
}
...
export const SubLink = Node.create<SubLinkOptions>({
  ...
  addOptions () {
    return {
      HTMLAttributes: {},
      hasHanzoText: (text: string) => false,
      hasHanzoLink: (href: string) => false
    }
  },
    ...
    return [
      {
        ...
        getAttrs: (el: HTMLElement | string) => {
          if (typeof el !== 'string') {
            if (this.options.hasHanzoText(el.textContent ?? '')) {
              return null
            }

            const link = el.querySelector('a[href]')
            const href = link?.getAttribute('href') ?? ''
            if (link != null && this.options.hasHanzoLink(href)) {
              return null
            }
          }
          // no match
