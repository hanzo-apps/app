 ...
 *
 * THIRD-PARTY FORGES ONLY. Our own git used to be a third provider here, pushing
 * to `api.hanzo.ai/v1/git/<org>/<slug>` — while the coding agent and the per-turn
 * commit wrote to `git.hanzo.ai/<user>/<slug>` through `lib/git/forge.ts`. One
 * project, two repos, two owners, two histories, and whichever one a person
 * looked at was missing half their work. git.hanzo.ai is the home; Publish goes
 * there through `forge.ts` like everything else, and this module is what it
 * always should have been: the EXPORT path to somebody else's forge.
 *
 ...
 *
 * READS ONLY. Nothing writes here any more — a project's home is git.hanzo.ai
 * and `lib/git/forge.ts` is the one client that writes to it.
 */
...
export interface PushOptions {
  /** Somebody else's forge. Ours is `lib/git/forge.ts`, and the type says so. */
  provider: Exclude<GitProvider, 'hanzo'>;
